...
const HelixMainEditorContainer = () => {
    ...
    useEffect(() => {
        if (selection?.state.selection.selectedPile) {
            const oldHelix = getHelixObjectFromPileId(helices?.state.helices as Helices[], selection?.state.selection.selectedPile.id)[0]
            setNumber(oldHelix.helices.length);
            setDiameter(oldHelix.helices[0].diameter);
            setFirstDist(oldHelix.distanceFromBottom);
            setSpacing(oldHelix.spacing as number);
        } else {
            setNumber(0)
            setDiameter(0)
            setFirstDist(0)
    ...
    useEffect(() => {
        if (selection?.state.selection.selectedPile) {
            const newHelix = getHelixObjectFromPileId(helices?.state.helices as Helices[], selection?.state.selection.selectedPile.id)[0]
            ...
            // Set Helices Context
            const newHelices = helices?.state.helices as Helices[]
            ...
            // Update Pile Context
            const newPile = getPileObjectFromPileId(piles?.state.piles as Piles, selection?.state.selection.selectedPile.id)
            newPile.helices = newHelix;

            const newPiles = piles?.state.piles as Piles;
